...
* Year: 2026
* Venue / source: arXiv
* Link: https://arxiv.org/abs/2607.29440
* Date surfaced: 2026-08-03
* Why selected in one sentence: It is the clearest paper in today's batch that long-term multimodal memory should support actual computation over accumulated state rather than stop at semantic retrieval.

## Quick verdict

**Must read**

I inspected the arXiv HTML paper, especially the analytic memory construction, schema induction, memory-aware joint query planning, main results, ablations, and limitations. This is one of the strongest memory papers in the recent batch because the extra structure actually does work: it lets the system filter, aggregate, compare, and rank recurring observations instead of just fetching vaguely relevant history. The main limitation is also clear in the paper: extraction errors and missing fields can propagate into induced schemas, and the tool inventory is still manually specified rather than self-expanding.
...
### Inputs
The framework takes a long-term multimodal interaction history containing dialogue, visual observations, and contextual metadata, plus a current user query.

### Outputs
It outputs a memory-access plan, executes retrieval and analytic tool calls against the constructed memories, and returns a grounded answer.

### Training objective (loss)
There is no single new end-to-end loss that is the core contribution here. The contribution is the memory representation and query-routing framework rather than a new monolithic training recipe.

### Architecture / parameterization
AdaMM has two explicit memory components. Retrieval memory hierarchically organizes interactions for recall. Analytic memory extracts attribute-value observations with provenance, induces recurring schemas, materializes them into executable structures, and exposes operation-specific access tools. A planner decides how to compose those tools at inference time.
...
If the memory layer stores only summaries or vector-retrievable chunks, the answer model has to reconstruct tables, compare states, and track temporal changes implicitly at answer time. The paper wants that burden moved into the memory representation itself.
...
It is evaluated against representative unimodal and multimodal memory baselines across multiple answer formats and task types, with ablations that remove analytic memory, retrieval memory, or the planning context.
...
The paper attacks a real failure mode, uses provenance-linked extraction instead of free-floating tables, gets solid gains across two benchmarks and two backbones, and shows via task breakdowns that the improvements are largest exactly where explicit analytics should matter.
...
Confidence-aware extraction, cross-round consistency checks, and safely extending the analytic tool inventory remain open. More generally, the field still lacks good self-evolving mechanisms for deciding what new operations persistent memory should learn.

### 11. What future work naturally follows?
Better uncertainty handling in record extraction, dynamic tool synthesis for new analytic operations, and memory benchmarks that stress structured updates under real multimodal noise would all be natural next steps.
...
Use provenance-linked attribute-value extraction instead of summary-only memory. Induce recurring schemas from repeated observations rather than baking one application schema in advance. Route queries explicitly between retrieval and analytic operations instead of hoping one memory interface can do both.
