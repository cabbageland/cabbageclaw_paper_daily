...
* Year: 2026
* Venue / source: arXiv
* Link: https://arxiv.org/abs/2607.27069
* Date surfaced: 2026-07-30
* Why selected in one sentence: It shows that a correct multimodal spatial answer can still be weakly image-supported, and it gives a cleaner audit than raw accuracy for separating those cases.
...
### Inputs
The audit takes a multimodal model, benchmark images and spatial questions, matched text-only and blank controls, and relation-contrast interventions.

### Outputs
The outputs are decision-level support and response metrics, including dependence-credited correctness and relation-response diagnostics.

### Training objective (loss)
There is no new trainable model here. VCA is a post-hoc audit procedure built from model scores and controlled input comparisons.

### Architecture / parameterization
The method works by comparing the model's score margins under the original image, no-image controls, matched image permutations, and fixed-pixel relation interventions. Its key parameterization choice is to separate marginal image support from relation-specific visual response instead of collapsing them into one metric.
...
Accuracy can hide shortcutting. If the model already prefers the answer without the image, then a correct answer on the original item does not prove multimodal spatial reasoning.
...
It compares original-image behavior against text-only and blank controls, uses matched same-split image permutation to calibrate the dependence metric, and adds a 3 by 3 evidence-source factorial plus relation-reversal tests to measure whether the model responds to the relevant visual evidence.
...
The method is post-hoc, does not require retraining, and uses controls that answer different identification questions instead of pretending one null condition is enough. That makes it much more useful than generic "accuracy drop under ablation" arguments.
...
A natural open problem is extending this style of audit to open-ended multimodal generation, richer spatial tasks, and other modalities where the control structure is harder to define cleanly.

### 11. What future work naturally follows?
Future work should use similar credit audits for benchmark filtering, training-data curation, and regression testing when new multimodal models claim better grounding.
...
Audit support-qualified correctness instead of raw correctness alone. Use matched permutation controls and relation-opposite interventions separately. Treat benchmark success as a decomposition problem rather than a single scalar.
